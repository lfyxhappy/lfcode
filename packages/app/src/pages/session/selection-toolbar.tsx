...
export function SelectionToolbar(props: {
  ...
  onAddToChat: (input: { text: string; comment?: string; messageID?: string; sessionID?: string }) => void
  ...
  const runAction = (action: (current: SelectionInfo) => false | void) => {
    const current = selection() ?? selectionInfo(props.roots())
    if (!current) return
    if (action(current) !== false) clear()
  }

  const runMouseAction =
    (action: (current: SelectionInfo) => false | void) =>
    (event: MouseEvent) => {
  ...
  return (
    <Portal>
      <div
        ref={bindToolbar}
        data-component="selection-toolbar"
        class="fixed z-50 inline-flex h-8 select-none items-center rounded-lg border border-border-base bg-surface-raised-stronger-non-alpha px-2.5 shadow-[var(--shadow-xs-border)]"
        style={{
        ...
        onMouseDown={(event) => {
          toolbarInteraction = true
          if (!commenting()) event.preventDefault()
          event.stopPropagation()
        }}
      >
        <Show
        ...
        >
          <input
            ref={commentInput}
            value={comment()}
            class="w-64 min-w-0 bg-transparent px-1 text-13-regular text-text-base outline-none placeholder:text-text-weak"
            placeholder={language.t("session.selection.commentPlaceholder")}
          ...
          <IconButton
            ...
            aria-label={language.t("session.selection.sendComment")}
