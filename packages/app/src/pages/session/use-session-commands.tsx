...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const activeBrowser = () => {
    ...
    const id = tab ? browserTabID(tab) : undefined
    if (!id) return
    const browser = layout.view(sessionKey()).browser.get(id)
    if (!browser) return
    return { id, browser }
  }
  ...
  const closeTab = () => {
    ...
    if (!tab) return
    const browser = browserTabID(tab)
    if (browser) {
      layout.view(sessionKey()).browser.close(browser)
    }
    tabs().close(tab)
  }
  ...
  const browserBack = () => {
    ...
    if (!active) return
    window.dispatchEvent(new CustomEvent(BROWSER_COMMAND_EVENT, { detail: { tabID: active.id, command: "back" } }))
  }

  const browserForward = () => {
    const active = activeBrowser()
    if (!active) return
    window.dispatchEvent(new CustomEvent(BROWSER_COMMAND_EVENT, { detail: { tabID: active.id, command: "forward" } }))
  }

  const browserReload = () => {
    const active = activeBrowser()
    if (!active) return
    window.dispatchEvent(new CustomEvent(BROWSER_COMMAND_EVENT, { detail: { tabID: active.id, command: "reload" } }))
  }

  const browserFocusAddress = () => {
    const active = activeBrowser()
    if (!active) return
    window.dispatchEvent(new CustomEvent(BROWSER_COMMAND_EVENT, { detail: { tabID: active.id, command: "focusAddress" } }))
  }

  const browserClose = () => {
    const active = activeBrowser()
    if (!active) return
    layout.view(sessionKey()).browser.close(active.id)
    tabs().close(browserTab(active.id))
  }

  const browserCopyAddress = async () => {
    const active = activeBrowser()
    if (!active) return
    const ok = await write(active.browser.url)
    if (!ok) {
      showToast({
        title: language.t("toast.browser.copyFailed.title"),
        variant: "error",
      })
      return
    }
    showToast({
      title: language.t("toast.browser.copySuccess.title"),
      description: active.browser.url,
      variant: "success",
    })
  }

  const browserOpenExternal = () => {
    const active = activeBrowser()
    if (!active) return
    if (platform.openExternalLink) {
      platform.openExternalLink(active.browser.url)
      return
    }
    window.open(active.browser.url, "_blank", "noopener,noreferrer")
  }
  ...
  const fileCmds = () => [
    ...
    fileCommand({
      id: "browser.back",
      title: language.t("command.browser.back"),
      disabled: !activeBrowser()?.browser.canGoBack,
      onSelect: browserBack,
    }),
    fileCommand({
      id: "browser.forward",
      title: language.t("command.browser.forward"),
      disabled: !activeBrowser()?.browser.canGoForward,
      onSelect: browserForward,
    }),
    fileCommand({
      id: "browser.reload",
      title: language.t("command.browser.reload"),
      disabled: !activeBrowser(),
      onSelect: browserReload,
    }),
    fileCommand({
      id: "browser.focusAddress",
      title: language.t("command.browser.focusAddress"),
      keybind: "mod+l",
      disabled: !activeBrowser(),
      onSelect: browserFocusAddress,
    }),
    fileCommand({
      id: "browser.copyAddress",
      title: language.t("command.browser.copyAddress"),
      disabled: !activeBrowser(),
      onSelect: () => {
        void browserCopyAddress()
      },
    }),
    fileCommand({
      id: "browser.openExternal",
      title: language.t("command.browser.openExternal"),
      disabled: !activeBrowser(),
      onSelect: browserOpenExternal,
    }),
    fileCommand({
      id: "browser.close",
      title: language.t("command.browser.close"),
      disabled: !activeBrowser(),
      onSelect: browserClose,
    }),
  ]
