...
function readGenericErrorMessage(error: unknown) {
  if (typeof error !== "object" || error === null) return
  const value = error as Record<string, unknown>
  if (typeof value.message === "string" && value.message) return value.message
  if (typeof value.formatted === "string" && value.formatted) return value.formatted
  if (typeof value.data !== "object" || value.data === null) return
  const data = value.data as Record<string, unknown>
  if (typeof data.message === "string" && data.message) return data.message
  if (typeof data.formatted === "string" && data.formatted) return data.formatted
}
...
export function formatServerError(error: unknown, translate?: Translator, fallback?: string) {
  ...
  const message = readGenericErrorMessage(error)
...
function parseReadableProviderModelNotFoundError(errorInput: ProviderModelNotFoundError, translator?: Translator) {
  ...
  const tail = tr(translator, "error.chain.checkConfig", "Check your config (lfcode.json) provider/model names")
  if (list.length) {
