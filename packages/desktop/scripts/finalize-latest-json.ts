...
const releaseId = process.env.LFCODE_RELEASE
if (!releaseId) throw new Error("LFCODE_RELEASE is required")

const version = process.env.LFCODE_VERSION
if (!version) throw new Error("LFCODE_VERSION is required")
...
const yver = winx?.version ?? wina?.version
