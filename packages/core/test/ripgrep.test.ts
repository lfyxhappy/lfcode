...
import { Ripgrep } from "@lfcode-ai/core/ripgrep"
import { RelativePath } from "@lfcode-ai/core/schema"
...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".lfcode")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".lfcode", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".lfcode/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".lfcode/config"))
