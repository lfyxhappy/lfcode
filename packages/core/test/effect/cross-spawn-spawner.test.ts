...
import { CrossSpawnSpawner } from "@lfcode-ai/core/cross-spawn-spawner"
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["LFCODE_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { LFCODE_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("LFCODE_TEST_SHELL=ok")
      }),
    )
