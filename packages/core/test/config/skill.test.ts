...
import { ConfigSkillPlugin } from "@lfcode-ai/core/config/plugin/skill"
...
import { SkillV2 } from "@lfcode-ai/core/skill"
    ...
    Effect.gen(function* () {
      ...
      yield* ConfigSkillPlugin.Plugin.effect.pipe(
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.lfcode") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        new SkillV2.DirectorySource({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.lfcode", "skill")),
        }),
        new SkillV2.DirectorySource({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.lfcode", "skills")),
        }),
