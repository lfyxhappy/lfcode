...
import { ConfigProvider } from "@lfcode-ai/core/config/provider"
import { ConfigMigrateV1 } from "@lfcode-ai/core/v1/config/migrate"
import { ConfigV1 } from "@lfcode-ai/core/v1/config/config"
import { FSUtil } from "@lfcode-ai/core/fs-util"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "lfcode.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "lfcode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "lfcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "lfcode.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "lfcode.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .lfcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".lfcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".lfcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "lfcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "lfcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "lfcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "lfcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".lfcode", "lfcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".lfcode", "lfcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".lfcode")),
              AbsolutePath.make(path.join(directory, ".lfcode")),
            ])
