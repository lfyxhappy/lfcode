...
import { InstructionContext } from "@lfcode-ai/core/instruction-context"
...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.LFCODE_DISABLE_PROJECT_CONFIG
      let scanned = false
      ...
      yield* SystemContextRegistry.Service.asEffect().pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.LFCODE_DISABLE_PROJECT_CONFIG
            else process.env.LFCODE_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
