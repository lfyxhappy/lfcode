...
import { AbsolutePath, Location, Model, Lfcode, Session, Tool } from "@lfcode-ai/core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(Lfcode.layer)

describe("public native Lfcode API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const opencode = yield* Lfcode.Service
...
const writeProvider = (directory: string, disabled = false) =>
  Effect.promise(() =>
    fs.writeFile(
      path.join(directory, "lfcode.json"),
      JSON.stringify({
        providers: {
          "public-test": {
            name: "Public test",
