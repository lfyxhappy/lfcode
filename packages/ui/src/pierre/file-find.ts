...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("lfcode-find")
  api.delete("lfcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("lfcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("lfcode-find", new Highlight(...rest))
    return true
  }
