---
title: Server
description: Interager med lfcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `lfcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en lfcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `lfcode.local` |
...
Indstil `LFCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `lfcode`, eller indstil `LFCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `lfcode serve` og `lfcode web`.
...
Når du kører `lfcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug lfcode-serveren til at interagere med lfcode programmatisk.
:::

Denne arkitektur lader lfcode understøtte flere klienter og lader dig interagere med lfcode programmatisk.

Du kan køre `lfcode serve` for at starte en selvstændig server. Hvis du har
lfcode TUI kørende, vil `lfcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Lfcode [IDE](/docs/ide) plugins.
...
Lfcode-serveren viser følgende API'er.
