---
title: Plugins
description: Skriv dine egne plugins for at udvide Lfcode.
---

Plugins giver dig mulighed for at udvide Lfcode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Lfcodes standardadfærd.
...
- `.lfcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/lfcode/node_modules/`.
...
1. Global konfiguration (`~/.config/lfcode/lfcode.json`)
2. Projektkonfiguration (`lfcode.json`)
3. Global plugin-mappe (`~/.config/lfcode/plugins/`)
4. Projekt plugin bibliotek (`.lfcode/plugins/`)
...
Lfcode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En lfcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide lfcode.
...
:::note
Hvis du bruger Lfcode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå lfcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til lfcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som lfcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for lfcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://lfcode.ai/docs/sdk) for detaljer.
