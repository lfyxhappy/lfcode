...
There are two types of agents in Lfcode; primary agents and subagents.

---

### Primary agents

Primary agents are the main assistants you interact with directly. You can cycle through them using the **Tab** key, or your configured `switch_agent` keybind. These agents handle your main conversation. Tool access is configured via permissions 鈥?for example, Build has all tools enabled while Plan is restricted.

:::tip
You can use the **Tab** key to switch between primary agents during a session.
:::

Lfcode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Lfcode comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Lfcode comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Lfcode's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `lfcode.json` config file:
...
- Global: `~/.config/lfcode/agents/`
- Per-project: `.lfcode/agents/`

```markdown title="~/.config/lfcode/agents/review.md"
---
...
If no temperature is specified, Lfcode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Lfcode config and the project specific config.
...
:::tip
If you don鈥檛 specify a model, primary agents use the [model globally configured](/docs/config#models) while subagents will use the model of the primary agent that invoked the subagent.
:::
...
The model ID in your Lfcode config uses the format `provider/model-id`. For example, if you're using [Lfcode Zen](/docs/zen), you would use `lfcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="lfcode.json" {3-6,9-12}
{
  "$schema": "https://lfcode.ai/config.json",
  "tools": {
    "write": true,
...
- `"ask"` 鈥?Prompt for approval before running the tool
- `"allow"` 鈥?Allow all operations without approval
- `"deny"` 鈥?Disable the tool
...
`read`, `edit`, `glob`, `grep`, `list`, `bash`, `task`, `external_directory`, `lsp`, and `skill` accept either a shorthand action (`"allow" | "ask" | "deny"`) or an object of glob/pattern 鈫?action for fine-grained control. The remaining keys accept the shorthand action only.

:::note
Permission keys are matched as wildcard patterns against the underlying tool name, so the same syntax works for built-ins, custom tools, and MCP tools 鈥?for example `"mymcp_*": "deny"` denies every tool from an MCP server, and `"mymcp_search": "ask"` targets a single one.
:::
...
```json title="lfcode.json" {3-5,8-10}
{
  "$schema": "https://lfcode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `lfcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/lfyxhappy/lfcode).
:::
...
```markdown title="~/.config/lfcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/lfcode/agents/security-auditor.md"
---
