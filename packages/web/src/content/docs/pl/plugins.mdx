---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć lfcode.
---

Wtyczki umożliwiające wyłączenie lfcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania lfcode.
...
- `.lfcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/lfcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/lfcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/lfcode/lfcode.json`)
2. Konfiguracja projektu (`lfcode.json`)
3. Globalny katalog wtyczek (`~/.config/lfcode/plugins/`)
4. Katalog wtyczek projektu (`.lfcode/plugins/`)
...
lfcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z lfcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do lfcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej lfcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj lfcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do lfcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod lfcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://lfcode.ai/docs/sdk).
