...
Aby debugować problemy z lfcode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\lfcode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `lfcode --log-level DEBUG`.

---

## Przechowywanie danych

lfcode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/lfcode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\lfcode`
...
lfcode Desktop uruchamia w tle lokalny serwer lfcode (sidecar `lfcode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Lfcode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/lfcode/lfcode.jsonc` (lub `~/.config/lfcode/lfcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/lfcode/lfcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\lfcode\lfcode.jsonc`
...
lfcode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/lfcode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\lfcode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby lfcode mógł ją odbudować.

1. Całkowicie zamknij lfcode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/lfcode`
- **Linux**: usuń `~/.cache/lfcode` (lub uruchom `rm -rf ~/.cache/lfcode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\lfcode`

3. Uruchom ponownie lfcode Desktop.

---

### Napraw problemy z połączeniem z serwerem

lfcode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `lfcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `LFCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `LFCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows lfcode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami lfcode.

---

### Brak powiadomień

lfcode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla lfcode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie lfcode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji lfcode Desktop):

- `lfcode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `lfcode.global.dat` i `lfcode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `lfcode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `lfcode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\lfcode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. lfcode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\lfcode`

2. Uruchom ponownie kod lfcode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to lfcode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
lfcode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
