...
Możesz dodać dodatkowe informacje do lfcode za pomocą _Model Context Protocol_, w skrócie MCP. lfcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [lfcode Config](https://lfcode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/lfcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
lfcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, lfcode:
...
Jeśli serwer wymaga uwierzytelnienia, lfcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `lfcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji lfcode bezpieczne przechowa tokeny w `~/.local/share/lfcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w lfcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie lfcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć lfcode z kontem Sentry.
