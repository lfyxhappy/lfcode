---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w lfcode.
---

import config from "../../../../config.mjs"
export const console = config.console

lfcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji lfcode.
...
w `~/.local/share/lfcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim lfcode
config.
...
Lfcode Zen to lista modeli dostarczonych przez zespół lfcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z lfcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Lfcode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Lfcode Zen` i przejdź do [lfcode.ai/auth](https://lfcode.ai/zen).
...
Działa jak każdy inny dostawca w lfcode i jest całkowicie opcjonalny w użyciu.

---

## Lfcode Go

Lfcode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół lfcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z lfcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Lfcode Go` i przejdź do [lfcode.ai/auth](https://lfcode.ai/zen).
...
Aby używać Amazon Bedrock z lfcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania lfcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `lfcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w lfcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować lfcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod lfcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji lfcode.
...
lfcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować lfcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`lfcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [lfcode-gitlab-auth](https://www.npmjs.com/package/lfcode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem lfcode:
...
Aby używać Google Vertex AI z lfcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania lfcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą lfcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [lfcode-helicone-session](https://github.com/H2Shami/lfcode-helicone-session), aby automatycznie rejestrować każdą konwersację lfcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja lfcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować lfcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować lfcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować lfcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem lfcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/lfcode).
:::
...
Aby korzystać z Ollama Cloud z lfcode:
...
3. Skopiuj klucz API do użycia w lfcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w lfcode musisz pobrać informacje o modelu lokalnie:
...
Lfcode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół lfcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Lfcode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Lfcode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API lfcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji lfcode.
...
5. Możesz je także dostosować za pomocą konfiguracji lfcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z lfcode:
...
Możesz także dostosować modele za pomocą konfiguracji lfcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z lfcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `lfcode.json` w katalogu projektu:
...
Pola `limit` pozwalają lfcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `lfcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację lfcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji lfcode.
