...
Umiejętności agenta pozwalają lfcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
lfcode przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu lfcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.lfcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/lfcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.lfcode/skills/git-release/SKILL.md` w ten sposób:
...
lfcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `lfcode.json`:
...
**Dla agentów wbudowanych** (w `lfcode.json`):
