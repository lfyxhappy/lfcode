...
Dzięki lfcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie lfcode używa naszego własnego motywu `lfcode`.
...
lfcode ma kilka wbudowanych motywów.
...
- Chcą, aby Lfcode pasował do wyglądu ich terminala
...
lfcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/lfcode/themes/*.json` lub `$XDG_CONFIG_HOME/lfcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.lfcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.lfcode/themes/*.json`
