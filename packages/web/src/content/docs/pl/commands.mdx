...
Utwórz `.lfcode/commands/test.md`:
...
Możesz zdefiniować polecenia w `lfcode.json` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji lfcode:
...
- Globalnie: `~/.config/lfcode/commands/`
- Na projekt: `.lfcode/commands/`
...
lfcode zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
