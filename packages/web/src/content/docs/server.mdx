---
title: Server
description: Interact with lfcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `lfcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an lfcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `lfcode.local` |
...
Set `LFCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `lfcode`, or set `LFCODE_SERVER_USERNAME` to override it. This applies to both `lfcode serve` and `lfcode web`.
...
When you run `lfcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the lfcode server to interact with lfcode programmatically.
:::

This architecture lets lfcode support multiple clients and allows you to interact with lfcode programmatically.

You can run `lfcode serve` to start a standalone server. If you have the
lfcode TUI running, `lfcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Lfcode [IDE](/docs/ide) plugins.
...
The lfcode server exposes the following APIs.
...
- `query` (required) 鈥?search string (fuzzy match)
- `type` (optional) 鈥?limit results to `"file"` or `"directory"`
- `directory` (optional) 鈥?override the project root for the search
- `limit` (optional) 鈥?max results (1鈥?00)
- `dirs` (optional) 鈥?legacy flag (`"false"` returns only files)
