---
title: Server
description: Interagisci con il server lfcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `lfcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client lfcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `lfcode.local` |
...
Imposta `LFCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `lfcode`, oppure imposta `LFCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `lfcode serve` sia per `lfcode web`.
...
Quando esegui `lfcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server lfcode per interagire con lfcode in modo programmatico.
:::

Questa architettura permette a lfcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `lfcode serve` per avviare un server standalone. Se la TUI di lfcode e gia in esecuzione, `lfcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Lfcode.
...
Il server lfcode espone le seguenti API.
