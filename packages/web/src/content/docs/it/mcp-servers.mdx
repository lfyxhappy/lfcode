...
Puoi aggiungere strumenti esterni a Lfcode usando il _Model Context Protocol_, o MCP. Lfcode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Lfcode Config](https://lfcode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/lfcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Lfcode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Lfcode:
...
Se il server richiede autenticazione, Lfcode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `lfcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Lfcode memorizzera' i token in modo sicuro in `~/.local/share/lfcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Lfcode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Lfcode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Lfcode al tuo account Sentry.
