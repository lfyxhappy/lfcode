---
title: Plugin
description: Scrivi plugin per estendere Lfcode.
---

I plugin ti permettono di estendere Lfcode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Lfcode.
...
- `.lfcode/plugins/` - plugin a livello progetto
- `~/.config/lfcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/lfcode/node_modules/`.
...
1. Config globale (`~/.config/lfcode/lfcode.json`)
2. Config di progetto (`lfcode.json`)
3. Directory plugin globale (`~/.config/lfcode/plugins/`)
4. Directory plugin di progetto (`.lfcode/plugins/`)
...
Lfcode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di lfcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere lfcode.
...
:::note
Se usi l'app desktop di Lfcode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a lfcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a lfcode:
...
L'helper `tool` crea uno strumento personalizzato che lfcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in lfcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://lfcode.ai/docs/sdk) per i dettagli.
