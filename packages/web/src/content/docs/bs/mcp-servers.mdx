...
Možete dodati vanjske alate u Lfcode koristeći _Model Context Protocol_, ili MCP. Lfcode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Lfcode Config](https://lfcode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/lfcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Lfcode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Lfcode će:
...
Ako server zahtijeva autentifikaciju, Lfcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `lfcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Lfcode sigurno čuva tokene u `~/.local/share/lfcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Lfcode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Lfcode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Lfcode sa vašim Sentry nalogom.
