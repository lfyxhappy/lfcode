---
title: Server
description: Komunicirajte s lfcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `lfcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji lfcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `lfcode.local` |
...
Postavite `LFCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `lfcode`, ili postavite `LFCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `lfcode serve` i za `lfcode web`.
...
Kada pokrenete `lfcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite lfcode server za programsku interakciju sa lfcode-om.
:::

Ova arhitektura omogucava lfcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `lfcode serve` da startate standalone server. Ako je lfcode TUI vec pokrenut, `lfcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Lfcode [IDE](/docs/ide) pluginovi.
...
lfcode server izlaže sljedece API-je.
