---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Lfcode.
---

Dodaci vam omogućavaju da proširite Lfcode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Lfcode.
...
- `.lfcode/plugins/` - Dodaci na nivou projekta
- `~/.config/lfcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/lfcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/lfcode/lfcode.json`)
2. Konfiguracija projekta (`lfcode.json`)
3. Globalni direktorij dodataka (`~/.config/lfcode/plugins/`)
4. Direktorij dodataka projekta (`.lfcode/plugins/`)
...
Lfcode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: lfcode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Lfcode.
...
Spriječite lfcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u lfcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji lfcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za lfcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://lfcode.ai/docs/sdk) za detalje.
