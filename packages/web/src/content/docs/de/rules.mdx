---
title: Regeln
description: Lege eigene Anweisungen fuer lfcode fest.
---

Du kannst lfcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in lfcode den Befehl `/init` aus.
...
So kann lfcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht lfcode sie zu erweitern.
...
lfcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/lfcode/AGENTS.md` speichern.
Diese gelten in allen lfcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Lfcode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/lfcode/AGENTS.md` existiert)
...
Beim Start sucht lfcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/lfcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `lfcode.json` oder in `~/.config/lfcode/lfcode.json` angeben.
...
lfcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit lfcode.json

Empfohlen ist das `instructions`-Feld in der `lfcode.json`:
...
Du kannst lfcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `lfcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
