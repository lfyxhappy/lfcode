---
title: Server
description: Nutze den lfcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `lfcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein lfcode-client nutzen kann.
...
Setze `LFCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `lfcode`, kann aber mit `LFCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `lfcode serve` und `lfcode web`.
...
Wenn du `lfcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den lfcode-Server, um lfcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `lfcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `lfcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Lfcode-[IDE](/docs/ide)-Plugins.
...
Der lfcode-Server stellt folgende APIs bereit.
