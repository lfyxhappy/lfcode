...
You can add external tools to Lfcode using the _Model Context Protocol_, or MCP. Lfcode supports both local and remote servers.
...
You can define MCP servers in your [Lfcode Config](https://lfcode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/lfcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Lfcode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Lfcode will:
...
If the server requires authentication, Lfcode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `lfcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Lfcode will store the tokens securely in `~/.local/share/lfcode/mcp-auth.json`.
...
Your MCPs are available as tools in Lfcode, alongside built-in tools. So you can manage them through the Lfcode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Lfcode to your Sentry account.
