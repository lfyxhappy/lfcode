...
description: lfcode dile özgü biçimlendiriciler kullanır.
---

lfcode, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

lfcode, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, lfcode bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

lfcode bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri lfcode yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
