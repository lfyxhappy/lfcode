...
_Model Bağlam Protokolü_ veya MCP kullanarak lfcode'a harici araçlar ekleyebilirsiniz. lfcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [lfcode Config](https://lfcode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/lfcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
lfcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde lfcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa lfcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `lfcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından lfcode, belirteçleri `~/.local/share/lfcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra lfcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi lfcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve lfcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
