---
title: Sunucu
description: lfcode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`lfcode serve` komutu, lfcode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `lfcode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `LFCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `lfcode` değeridir; değiştirmek isterseniz `LFCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `lfcode serve` hem de `lfcode web` için geçerlidir.
...
`lfcode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
lfcode ile programatik etkileşim için lfcode sunucusunu kullanın.
:::

Bu mimari, lfcode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`lfcode serve` ile bağımsız bir sunucu başlatabilirsiniz. lfcode TUI açıksa, `lfcode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum lfcode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
lfcode sunucusu aşağıdaki API'leri sunar.
