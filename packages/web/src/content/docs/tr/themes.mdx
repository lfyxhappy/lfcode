...
lfcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak lfcode kendi `lfcode` temasını kullanır.
...
lfcode birden fazla yerleşik temayla gelir.
...
- lfcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
lfcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/lfcode/themes/*.json` veya `$XDG_CONFIG_HOME/lfcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.lfcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.lfcode/themes/*.json`
