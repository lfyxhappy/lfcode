...
`.lfcode/commands/test.md` oluştur:
...
lfcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

lfcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/lfcode/commands/`
- Proje başına: `.lfcode/commands/`
...
lfcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
