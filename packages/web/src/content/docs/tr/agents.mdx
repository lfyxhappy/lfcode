...
lfcode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
lfcode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Lfcode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Lfcode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `lfcode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse lfcode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global lfcode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
lfcode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Lfcode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `lfcode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `lfcode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/lfyxhappy/lfcode).
:::
