...
Ajan becerileri, lfcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
lfcode su konumlari tarar:

- Proje konfigurasyonu: `.lfcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/lfcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda lfcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.lfcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/lfcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.lfcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
lfcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`lfcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`lfcode.json` icinde):
