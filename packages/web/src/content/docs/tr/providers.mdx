---
title: Sağlayıcılar
description: lfcode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

lfcode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı lfcode yapılandırmanızda yapılandırın.
...
`~/.local/share/lfcode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları lfcode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Lfcode Zen, lfcode ekibi tarafından sağlanan modellerin bir listesidir.
lfcode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Lfcode Zen ile başlamanızı öneririz.
:::
...
lfcode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Lfcode Go

Lfcode Go, lfcode ile iyi çalıştığı test edilmiş ve doğrulanmış, lfcode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Lfcode Go`'yu seçin ve [lfcode.ai/auth](https://lfcode.ai/zen) adresine gidin.
...
Amazon Bedrock'u lfcode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   lfcode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `lfcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi lfcode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
lfcode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   lfcode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   lfcode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
lfcode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. lfcode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`lfcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [lfcode-gitlab-auth](https://www.npmjs.com/package/lfcode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`lfcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi lfcode'la kullanmak için:
...
Google Vertex AI'yi lfcode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları lfcode çalıştırılırken ayarlayın.
...
Helicone'un lfcode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her lfcode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [lfcode-helicone-session](https://github.com/H2Shami/lfcode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her lfcode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
lfcode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla lfcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla lfcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini lfcode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/lfcode)'a bakın.
:::
...
Ollama Cloud'u lfcode ile kullanmak için:
...
3. lfcode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: lfcode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Lfcode Zen, lfcode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Lfcode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Lfcode Zen**'i arayın.

   ```txt
   /connect
   ```

3. lfcode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   lfcode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca lfcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi lfcode ile kullanmak için:
...
Modelleri lfcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı lfcode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `lfcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları lfcode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `lfcode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için lfcode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, lfcode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
