...
lfcode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\lfcode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `lfcode --log-level DEBUG`.

---

## Depolama

lfcode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/lfcode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\lfcode` yapistirin
...
lfcode Desktop arka planda yerel bir lfcode sunucusu (`lfcode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `lfcode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/lfcode/lfcode.jsonc` (veya `~/.config/lfcode/lfcode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/lfcode/lfcode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\lfcode\lfcode.jsonc` yapistirin
...
lfcode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/lfcode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\lfcode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), lfcode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. lfcode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/lfcode`
- **Linux**: `~/.cache/lfcode` dizinini silin (veya `rm -rf ~/.cache/lfcode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\lfcode` yapistirin

3. lfcode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

lfcode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`lfcode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `LFCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `LFCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta lfcode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, lfcode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

lfcode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda lfcode icin bildirimler etkinse
...
1. lfcode Desktop'u kapatin
2. Su dosyalari bulun ve silin (lfcode Desktop uygulama veri dizininde yer alirlar):

- `lfcode.settings.dat` (desktop varsayilan sunucu URL'si)
- `lfcode.global.dat` ve `lfcode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
lfcode ile ilgili bir sorun yasiyorsaniz:
...
3. `lfcode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `lfcode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\lfcode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. lfcode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\lfcode`

2. En guncel provider paketlerini yeniden kurmak icin lfcode'u yeniden baslatin

Bu, lfcode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
lfcode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
