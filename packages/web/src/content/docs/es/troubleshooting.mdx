...
Para depurar problemas con Lfcode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/lfcode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\lfcode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `lfcode --log-level DEBUG`.

---

## Almacenamiento

lfcode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/lfcode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\lfcode`
...
Lfcode Desktop ejecuta un servidor Lfcode local (el sidecar `lfcode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Lfcode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/lfcode/lfcode.jsonc` (o `~/.config/lfcode/lfcode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/lfcode/lfcode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\lfcode\lfcode.jsonc`
...
Lfcode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/lfcode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\lfcode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.lfcode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Lfcode pueda reconstruirlo.

1. Salga de Lfcode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/lfcode`
- **Linux**: eliminar `~/.cache/lfcode` (o ejecutar `rm -rf ~/.cache/lfcode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\lfcode`

3. Reinicie el escritorio Lfcode.

---

### Solucionar problemas de conexión del servidor

Lfcode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `lfcode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LFCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LFCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Lfcode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Lfcode.

---

### Notificaciones que no se muestran

Lfcode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Lfcode en la configuración de su sistema operativo, y
...
1. Salga del escritorio Lfcode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Lfcode):

- `lfcode.settings.dat` (URL del servidor predeterminado de escritorio)
- `lfcode.global.dat` y `lfcode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/lfcode/issues**](https://github.com/lfyxhappy/lfcode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**lfcode.ai/discord**](https://lfcode.ai/discord)
...
3. Asegúrese de tener la última versión con `lfcode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `lfcode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/lfcode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\lfcode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. lfcode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\lfcode`

2. Reinicie lfcode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a lfcode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
lfcode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
