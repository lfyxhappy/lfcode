...
description: Cree herramientas que LLM pueda llamar en lfcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de lfcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.lfcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/lfcode/tools/`.
...
```ts title=".lfcode/tools/database.ts" {1}
...
```ts title=".lfcode/tools/project.ts" {8}
import { tool } from "@lfcode-ai/plugin"
...
```python title=".lfcode/tools/add.py"
import sys
...
```ts title=".lfcode/tools/python-add.ts" {10}
import { tool } from "@lfcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".lfcode/tools/add.py")
