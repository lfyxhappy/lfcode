---
title: Servidor
description: Interactuar con el servidor lfcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `lfcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente lfcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `lfcode.local` |
...
Configure `LFCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `lfcode`, o configure `LFCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `lfcode serve` como a `lfcode web`.
...
Cuando ejecuta `lfcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor lfcode para interactuar con lfcode mediante programación.
:::

Esta arquitectura permite que lfcode admita múltiples clientes y le permite interactuar con lfcode mediante programación.

Puede ejecutar `lfcode serve` para iniciar un servidor independiente. Si tienes el
lfcode TUI ejecutándose, `lfcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Lfcode [IDE](/docs/ide).
...
El servidor lfcode expone las siguientes API.
