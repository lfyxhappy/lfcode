---
title: Plugins
description: Escreva seus próprios plugins para estender o lfcode.
---

Plugins permitem que você estenda o lfcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do lfcode.
...
- `.lfcode/plugins/` - Plugins em nível de projeto
- `~/.config/lfcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/lfcode/node_modules/`.
...
1. Configuração global (`~/.config/lfcode/lfcode.json`)
2. Configuração do projeto (`lfcode.json`)
3. Diretório de plugins global (`~/.config/lfcode/plugins/`)
4. Diretório de plugins do projeto (`.lfcode/plugins/`)
...
O lfcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do lfcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o lfcode.
...
:::note
Se você estiver usando o aplicativo desktop lfcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o lfcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao lfcode:
...
O helper `tool` cria uma ferramenta personalizada que o lfcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o lfcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://lfcode.ai/docs/sdk) para detalhes.
