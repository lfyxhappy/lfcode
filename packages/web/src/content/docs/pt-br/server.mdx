---
title: Servidor
description: Interaja com o servidor lfcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `lfcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente lfcode pode usar.
...
```bash
lfcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `lfcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
lfcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `LFCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `lfcode`, ou defina `LFCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `lfcode serve` quanto ao `lfcode web`.

```bash
LFCODE_SERVER_PASSWORD=your-password lfcode serve
```
...
Quando você executa `lfcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor lfcode para interagir com o lfcode programaticamente.
:::

Essa arquitetura permite que o lfcode suporte múltiplos clientes e permite que você interaja com o lfcode programaticamente.

Você pode executar `lfcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do lfcode em execução, `lfcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do lfcode [IDE](/docs/ide).
...
O servidor lfcode expõe as seguintes APIs.
