...
Com o lfcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o lfcode usa nosso próprio tema `lfcode`.
...
O lfcode vem com vários temas embutidos.
...
- Querem que o lfcode corresponda à aparência do seu terminal
...
O lfcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/lfcode/themes/*.json` ou `$XDG_CONFIG_HOME/lfcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.lfcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.lfcode/themes/*.json`
