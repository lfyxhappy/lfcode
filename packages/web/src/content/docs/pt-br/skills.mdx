...
As habilidades do agente permitem que o lfcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O lfcode pesquisa nesses locais:

- Configuração do projeto: `.lfcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/lfcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o lfcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.lfcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/lfcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.lfcode/skills/git-release/SKILL.md` assim:
...
O lfcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `lfcode.json`:
...
**Para agentes embutidos** (em `lfcode.json`):
