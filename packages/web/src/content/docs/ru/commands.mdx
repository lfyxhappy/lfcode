...
Создайте `.lfcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию lfcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем lfcode [config](/docs/config):
...
- Глобальный: `~/.config/lfcode/commands/`
- Для каждого проекта: `.lfcode/commands/`
...
lfcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
