---
title: Serveur
description: Interagissez avec le serveur lfcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `lfcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client lfcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `lfcode.local` |
...
Définissez `LFCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `lfcode`, ou définissez `LFCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `lfcode serve` et à `lfcode web`.
...
Lorsque vous exécutez `lfcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur lfcode pour interagir avec lfcode par programmation.
:::

Cette architecture permet à lfcode de prendre en charge plusieurs clients et vous permet d'interagir avec lfcode par programme.

Vous pouvez exécuter `lfcode serve` pour démarrer un serveur autonome. Si vous avez le
lfcode TUI en cours d'exécution, `lfcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Lfcode [IDE](/docs/ide).
...
Le serveur lfcode expose les API suivantes.
