---
title: Plugins
description: Écrivez vos propres plugins pour étendre Lfcode.
---

Les plugins vous permettent d'étendre Lfcode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Lfcode.
...
- `.lfcode/plugins/` - Plugins au niveau du projet
- `~/.config/lfcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/lfcode/node_modules/`.
...
1. Configuration globale (`~/.config/lfcode/lfcode.json`)
2. Configuration du projet (`lfcode.json`)
3. Répertoire global des plugins (`~/.config/lfcode/plugins/`)
4. Répertoire des plugins du projet (`.lfcode/plugins/`)
...
Lfcode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK lfcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre lfcode.
...
:::note
Si vous utilisez l'application de bureau Lfcode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez lfcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à lfcode :
...
L'assistant `tool` crée un outil personnalisé que lfcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour lfcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://lfcode.ai/docs/sdk) pour plus de détails.
