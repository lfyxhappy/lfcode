---
title: SDK
description: Client JS de type sécurisé pour le serveur lfcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK lfcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler lfcode par programme.
...
Créez une instance de lfcode :

```javascript
import { createlfcode } from "@lfcode-ai/sdk"

const { client } = await createLfcode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `lfcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${lfcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de lfcode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@lfcode-ai/sdk"
```
