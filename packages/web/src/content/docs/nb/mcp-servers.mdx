...
Du kan legge til eksterne verktøy i Lfcode ved å bruke _Model Context Protocol_ eller MCP. Lfcode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Lfcode Config](https://lfcode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="lfcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/lfcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Lfcode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Lfcode:
...
Hvis serveren krever autentisering, vil Lfcode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `lfcode mcp auth <server-name>`.
...
```bash
lfcode mcp auth my-oauth-server
```
...
```bash
lfcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Lfcode lagre tokens sikkert i `~/.local/share/lfcode/mcp-auth.json`.
...
lfcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Lfcode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Lfcode-konfigurasjonen som alle andre verktøy.
...
```json title="lfcode.json" {11, 14-18}
{
  "$schema": "https://lfcode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
lfcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Lfcode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Lfcode-samtale oppført som en separat økt.
