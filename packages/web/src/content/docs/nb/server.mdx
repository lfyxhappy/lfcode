---
title: Tjener
description: Samhandle med Lfcode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `lfcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Lfcode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `lfcode.local` |
...
Sett `LFCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `lfcode` som standard, eller sett `LFCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `lfcode serve` og `lfcode web`.
...
Når du kjører `lfcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Lfcode-serveren til å samhandle med Lfcode programmatisk.
:::

Denne arkitekturen lar Lfcode støtte flere klienter og lar deg samhandle med Lfcode programmatisk.

Du kan kjøre `lfcode serve` for å starte en frittstående server. Hvis du har
Lfcode TUI kjører, vil `lfcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Lfcode [IDE](/docs/ide) plugins.
...
lfcode-serveren viser følgende APIer.
