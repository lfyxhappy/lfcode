...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في lfcode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في lfcode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.lfcode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/lfcode/tools/`.
