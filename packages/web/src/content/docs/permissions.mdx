...
Lfcode uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
- `"allow"` 鈥?run without approval
- `"ask"` 鈥?prompt for approval
- `"deny"` 鈥?block the action
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Lfcode was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Lfcode permissions are keyed by tool name, plus a couple of safety guards:

- `read` 鈥?reading a file (matches the file path)
- `edit` 鈥?all file modifications (covers `edit`, `write`, `patch`)
- `glob` 鈥?file globbing (matches the glob pattern)
- `grep` 鈥?content search (matches the regex pattern)
- `bash` 鈥?running shell commands (matches parsed commands like `git status --porcelain`)
- `task` 鈥?launching subagents (matches the subagent type)
- `skill` 鈥?loading a skill (matches the skill name)
- `lsp` 鈥?running LSP queries (currently non-granular)
- `question` 鈥?asking the user questions during execution
- `webfetch` 鈥?fetching a URL (matches the URL)
- `websearch` 鈥?web search (matches the query)
- `external_directory` 鈥?triggered when a tool touches paths outside the project working directory
- `doom_loop` 鈥?triggered when the same tool call repeats 3 times with identical input

---

## Defaults

If you don鈥檛 specify anything, Lfcode starts from permissive defaults:
...
When Lfcode prompts for approval, the UI offers three outcomes:

- `once` 鈥?approve just this request
- `always` 鈥?approve future requests matching the suggested patterns (for the rest of the current Lfcode session)
- `reject` 鈥?deny the request
