---
title: Providers
description: Using any LLM provider in Lfcode.
---

import config from "../../../config.mjs"
export const console = config.console

Lfcode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Lfcode config.
...
in `~/.local/share/lfcode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Lfcode
config.
...
Lfcode Zen is a list of models provided by the Lfcode team that have been
tested and verified to work well with Lfcode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Lfcode Zen.
:::

1. Run the `/connect` command in the TUI, select `Lfcode Zen`, and head to [lfcode.ai/auth](https://lfcode.ai/zen).
...
It works like any other provider in Lfcode and is completely optional to use.

---

## Lfcode Go

Lfcode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Lfcode team that have been
tested and verified to work well with Lfcode.

1. Run the `/connect` command in the TUI, select `Lfcode Go`, and head to [lfcode.ai/auth](https://lfcode.ai/zen).
...
To use Amazon Bedrock with Lfcode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running lfcode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY lfcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile lfcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX lfcode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `lfcode.json`:
...
2. Here you can select the **Claude Pro/Max** option and it'll open your browser
   ...
   ```txt
   鈹?Select auth method
   鈹?   鈹?Manually enter API Key
   鈹?   ```
...
Lfcode. Anthropic explicitly prohibits this.

Previous versions of Lfcode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Lfcode with zero setup:
...
You can configure lfcode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="lfcode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://lfcode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
- `models` is a map of model IDs to their display names. Each ID must match the `id` returned by `GET /v1/models` 鈥?run `curl http://127.0.0.1:1337/v1/models` to list the ids currently loaded in Atomic Chat.
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for lfcode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX lfcode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX lfcode
   ```
...
3. Enter your **Account ID** when prompted.
   ...
   ```txt
   鈹?Enter your Cloudflare Account ID
   鈹?   鈹?   鈹?enter
   ```
...
4. Enter your **Gateway ID** when prompted.
   ...
   ```txt
   鈹?Enter your Cloudflare AI Gateway ID
   鈹?   鈹?   鈹?enter
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your lfcode config.
...
Lfcode supports two authentication methods:

- **OAuth (Recommended)** 鈥?Sign in to your DigitalOcean account; Lfcode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
- **Model Access Key** 鈥?Paste an existing key from the DigitalOcean console.
...
2. Select **Login with DigitalOcean**.
   ...
   ```txt
   鈹?Select auth method
   鈹?   鈹?Login with DigitalOcean
   鈹?Paste Model Access Key
   鈹?   ```

3. Your browser opens to authorize Lfcode. Sign in and approve.

   :::note
   Lfcode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference 鈥?no separate Model Access Key is created.
   :::
...
2. Run the `/connect` command and select **DigitalOcean**, then **Paste Model Access Key**.
   ...
   ```txt
   鈹?Enter your DigitalOcean Model Access Key
   鈹?   鈹?   鈹?enter
   ```
...
Inference Routers let you define a routing policy across multiple models 鈥?picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Lfcode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model 鈥?Lfcode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Lfcode is experimental. Features, configuration, and
behavior may change in future releases.
:::

Lfcode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   ```txt
   鈹?Select auth method
   鈹?   鈹?OAuth (Recommended)
   鈹?Personal Access Token
   鈹?   ```
...
to store token in lfcode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Lfcode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Lfcode
to only use your own GitLab-hosted instance, add the following to your
`lfcode.json` file. It is also recommended to disable session sharing.
...
a new application (Settings 鈫?Applications) with the
...
More documentation on [lfcode-gitlab-auth](https://www.npmjs.com/package/lfcode-gitlab-auth) homepage.

##### Configuration

Customize through `lfcode.json`:
...
When a `duo-workflow-*` model is selected, Lfcode will:
...
4. Route tool execution requests through Lfcode's permission-gated tool system
...
To use your GitHub Copilot subscription with lfcode:
...
2. Navigate to [github.com/login/device](https://github.com/login/device) and enter the code.
   ...
   ```txt
   鈹?Login with GitHub Copilot
   鈹?   鈹?https://github.com/login/device
   鈹?   鈹?Enter code: 8F43-6FCF
   鈹?   鈹?Waiting for authorization...
   ```
...
To use Google Vertex AI with Lfcode:
...
2. Set the required environment variables:
   ...
   Set them while running lfcode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id lfcode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through lfcode, you can always configure it yourself.
...
```jsonc title="~/.config/lfcode/lfcode.jsonc"
{
  "$schema": "https://lfcode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [lfcode-helicone-session](https://github.com/H2Shami/lfcode-helicone-session) plugin to automatically log each Lfcode conversation as a session in Helicone.

```bash
npm install -g lfcode-helicone-session
```
...
```json title="lfcode.json"
{
  "plugin": ["lfcode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Lfcode conversation listed as a separate session.
...
You can configure lfcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="lfcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://lfcode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure lfcode to use local models through LM Studio.

```json title="lfcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://lfcode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure lfcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Lfcode. See the [Ollama integration docs](https://docs.ollama.com/integrations/lfcode) for details.
:::

```json title="lfcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://lfcode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Lfcode:
...
3. Copy the API key for use in Lfcode.
...
6. **Important**: Before using cloud models in Lfcode, you must pull the model information locally:
...
2. Here you can select the **ChatGPT Plus/Pro** option and it'll open your browser
   ...
   ```txt
   鈹?Select auth method
   鈹?   鈹?ChatGPT Plus/Pro
   鈹?Manually enter API Key
   鈹?   ```
...
Lfcode Zen is a list of tested and verified models provided by the Lfcode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Lfcode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Lfcode Zen**.

   ```txt
   /connect
   ```

3. Enter your Lfcode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your lfcode config.
...
5. You can also customize them through your lfcode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' lfcode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group lfcode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with lfcode:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=lfcode&utm_medium=docs&utm_campaign=cortex-provider).

lfcode's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
3. Enter your [account identifier](https://docs.snowflake.com/en/user-guide/admin-account-identifier) when prompted (e.g. `myorg-myaccount` or `xy12345.us-east-1`).
   ...
   ```txt
   鈹?Snowflake Account Identifier
   鈹?   鈹?   鈹?enter
   ```
...
4. Enter your **PAT**.
   ...
   ```txt
   鈹?Programmatic Access Token (PAT)
   鈹?   鈹?   鈹?enter
   ```
...
Alternatively, set environment variables before starting lfcode:
...
The model catalog is provided automatically. A minimal `lfcode.json` is all that's needed:
...
You can also customize models through your lfcode config. Here's an example of specifying provider routing order.
...
#### Option A 鈥?SuperGrok OAuth (browser login)
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Lfcode opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Lfcode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B 鈥?SuperGrok device-code (headless / remote server / VPS)

Use this when Lfcode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Lfcode 鈥?instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, 鈥?.
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Lfcode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Lfcode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with lfcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   鈹? Add credential
   鈹?   鈻? This only stores a credential for myprovider - you will need to configure it in lfcode.json, check the docs for examples.
   鈹?   鈼? Enter your API key
   鈹? sk-...
   鈹?   ```

4. Create or update your `lfcode.json` file in your project directory:

   ```json title="lfcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://lfcode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="lfcode.json" {9,11,17-20}
{
  "$schema": "https://lfcode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Lfcode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `lfcode auth list` to see if the credentials
...
2. For custom providers, check the lfcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your lfcode config.
