import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@lfcode-ai/plugin/tui"
...
import type { useLfcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useLfcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getLfcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getLfcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
