...
import { getLfcodeModeStack, LFCODE_BASE_MODE, LfcodeKeymapProvider, registerLfcodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerLfcodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when Lfcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getLfcodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getLfcodeModeStack(keymap).push("autocomplete")
