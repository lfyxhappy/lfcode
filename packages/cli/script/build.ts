...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      LFCODE_CLI_NAME: `'${binary}'`,
      LFCODE_MODELS_DEV: modelsData,
      LFCODE_CHANNEL: `'${Script.channel}'`,
      LFCODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/lfyxhappy/lfcode.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
