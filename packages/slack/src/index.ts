...
import { createLfcode, type ToolPart } from "@lfcode-ai/sdk"
...
console.log("🚀 Starting Lfcode server...")
const lfcode = await createLfcode({
  port: 0,
})
console.log("✅ Lfcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await lfcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new Lfcode session...")
    const { client, server } = lfcode
    ...
    console.log("✅ Created Lfcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to Lfcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Lfcode response:", JSON.stringify(result, null, 2))
