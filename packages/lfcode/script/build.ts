...
const forceInstall = process.argv.includes("--install-target-deps")
const skipInstall = process.argv.includes("--skip-install") || (!forceInstall && !Script.release)
...
const singleTargetBuild = singleFlag || (!allTargetsFlag && !Script.release)
const targets = singleTargetBuild
...
if (!skipInstall) {
  ...
  console.log("Skipping cross-target dependency install for local build")
}
for (const item of targets) {
  ...
  await Bun.build({
    ...
    define: {
      ...
      LFCODE_CHANNEL: `'${Script.channel}'`,
      LFCODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
    },
  })

  // Smoke test: only run if binary is for current platform
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/lfcode`
