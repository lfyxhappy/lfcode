CREATE INDEX IF NOT EXISTS `usage_fact_time_idx` ON `usage_fact` (`time_created`, `part_id`);
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS `usage_fact_session_time_idx` ON `usage_fact` (`session_id`, `time_created`, `part_id`);
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS `usage_fact_project_time_idx` ON `usage_fact` (`project_id`, `time_created`, `part_id`);
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS `usage_fact_provider_model_idx` ON `usage_fact` (`provider_id`, `model_id`, `time_created`);
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS `usage_fact_status_idx` ON `usage_fact` (`status`, `time_created`);
--> statement-breakpoint
INSERT OR IGNORE INTO usage_fact (
...
SELECT
  p.id, p.message_id, p.session_id, s.project_id, p.time_created, m.agent_id,
...
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS `usage_fact_backfill` (
...
--> statement-breakpoint
INSERT OR IGNORE INTO `usage_fact_backfill` (`id`, `completed`, `updated_at`) VALUES (1, 1, unixepoch() * 1000);
