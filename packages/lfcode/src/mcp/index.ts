...
type OptionalListLabel = "prompts" | "resources"
...
function listSupported(client: Client, label: OptionalListLabel) {
  const capabilities = client.getServerCapabilities()
  if (label === "prompts") return !!capabilities?.prompts
  return !!capabilities?.resources
}

function listMarkedUnsupported(client: Client, label: OptionalListLabel) {
  return unsupportedLists.get(client)?.has(label) ?? false
}

function markListUnsupported(client: Client, label: OptionalListLabel) {
  const current = unsupportedLists.get(client) ?? new Set<OptionalListLabel>()
  current.add(label)
  unsupportedLists.set(client, current)
}
...
function fetchFromClient<T extends { name: string }>(
  ...
  label: OptionalListLabel,
) {
  if (!listSupported(client, label) || listMarkedUnsupported(client, label)) return Effect.succeed(undefined)
  return Effect.tryPromise({
    try: () => listFn(client),
    catch: (e) => {
      const message = e instanceof Error ? e.message : String(e)
      if (isMethodNotFoundError(e) || message.includes(`does not support ${label}`)) {
        markListUnsupported(client, label)
        log.debug(`${label} not supported by client`, { clientName, error: message })
        return e
      }
...
export class Service extends Context.Service<Service, Interface>()("@lfcode/MCP") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "lfcode", version: InstallationVersion })
            ...
            if (isAuthError) {
              ...
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return bus
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: lfcode mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const disconnect = Effect.fn("MCP.disconnect")(function* (name: string) {
      ...
      const mcp = yield* getMcpConfig(name)
      if (mcp) {
        yield* cfgSvc.updateMcpEnabled(name, false)
      }
      if (!mcp && !s.clients[name] && !s.status[name]) {
        log.error("MCP config not found or invalid", { name })
        return
      }
      yield* closeClient(s, name)
