...
const MANAGED_PLIST_DOMAIN = "ai.lfcode.managed"

// Keys injected by macOS/MDM into the managed plist that are not Lfcode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/lfcode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "lfcode")
    default:
      return "/etc/lfcode"
  }
}
...
export function managedConfigDir() {
  return process.env.LFCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
