...
import { supportsXiaomiWebsearch } from "./websearch/provider"
      ...
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        return {
          ...
          builtin: [
            ...
            tool.history,
            tool.task,
            ...(Flag.LFCODE_EXPERIMENTAL_WORKFLOW_TOOL ? [tool.workflow] : []),
          ],
          actor: tool.actor,
          read: tool.read,
        }
      ...
      let filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          if (tool.id === WebSearchTool.id) {
            return (
              input.providerID === ProviderID.lfcode ||
              supportsXiaomiWebsearch(input.providerID) ||
              Flag.LFCODE_ENABLE_EXA
            )
          }
