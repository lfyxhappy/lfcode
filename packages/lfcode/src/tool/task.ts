...
const parameters = z.strictObject({
  ...
  // Some providers stringify the entire envelope, producing
...
function normalizeTaskArgs(input: unknown) {
...
const taskParameters = z.preprocess(normalizeTaskArgs, parameters)
...
function formatTaskValidationError(error: z.ZodError) {
  ...
  return [
    ...
    details,
    'Use exactly one JSON envelope: {"operation":{"action":"...",...}}.',
    'For create use {"operation":{"action":"create","summary":"..."}}; the task id is generated by the tool, so do not provide operation.id.',
    "For get/start/block/unblock/done/abandon/rename, provide the existing task id in operation.id.",
    "Do not retry the same invalid JSON unchanged.",
  ].join("\n")
}

type TaskInput = z.infer<typeof taskParameters>
...
export const TaskTool = Tool.define<typeof taskParameters, Metadata, TaskRegistry.Service>(
  id,
  Effect.gen(function* () {
...
function contextReadGate(ctx: Tool.Context<Metadata>, operation: "list" | "get", sessionID: SessionID, taskID?: string) {
  ...
  return gate
    ...
    return {
      ...
      formatValidationError: formatTaskValidationError,
      execute: (args: z.infer<typeof taskParameters>, ctx: Tool.Context<Metadata>) =>
    ...
    } satisfies Tool.DefWithoutID<typeof taskParameters, Metadata>
  }),
)
