...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall lfcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Lfcode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @lfcode-ai/cli",
      pnpm: "pnpm uninstall -g @lfcode-ai/cli",
      bun: "bun remove -g @lfcode-ai/cli",
      // TODO(lfcode): uncomment when published to these channels
      // brew: "brew uninstall lfcode",
      // choco: "choco uninstall lfcode",
      // scoop: "scoop uninstall lfcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Lfcode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# lfcode") || content.includes(".lfcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".lfcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".lfcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".lfcode"))
    ) {
      continue
    }
