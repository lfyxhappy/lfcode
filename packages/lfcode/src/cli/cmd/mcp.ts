    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in lfcode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .lfcode/ subdirectory too)
  const candidates = [path.join(baseDir, "lfcode.json"), path.join(baseDir, "lfcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".lfcode", "lfcode.json"), path.join(baseDir, ".lfcode", "lfcode.jsonc"))
  }
  ...
  // Default to lfcode.json if none exist
  return candidates[0]
}
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "lfcode-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
