...
import { createLfcodeClient, type LfcodeClient, type ToolPart } from "@lfcode-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run lfcode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running lfcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to LFCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: LfcodeClient) {
    ...
    async function share(sdk: LfcodeClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.LFCODE_AUTO_SHARE && !args.share) return
        ...
        try {
          while (true) {
            ...
            if (event.type === "session.error") {
              ...
              if (props.visible === false || props.sessionID !== sessionID || !props.error) continue
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.LFCODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.LFCODE_SERVER_USERNAME ?? "lfcode"
        const auth = `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`
        return { Authorization: auth }
      })()
      const sdk = createLfcodeClient({ baseUrl: args.attach, directory, headers })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createLfcodeClient({ baseUrl: "http://lfcode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
