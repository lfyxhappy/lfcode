      ...
      async fn() {
        {
          ...
          if (!providerData) {
            throw new Error(`Selected provider "${provider}" not found`)
          }
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://lfcode.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function promptProvider(): Promise<string> {
            ...
            if (typeof selectedProvider === "string") return selectedProvider
...
jobs:
  lfcode:
    ...
    steps:
      ...
      - name: Run Lfcode
        uses: lfyxhappy/lfcode/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  // Setup lfcode session
    ...
    return `lfcode/${type}-${hex}-${timestamp}`
        }
        return `lfcode/${type}${issueId}-${timestamp}`
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the lfcode infrastructure after your response",
