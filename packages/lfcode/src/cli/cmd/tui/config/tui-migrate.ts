...
const TUI_SCHEMA_URL = "https://lfcode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from lfcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const lfcode = await lfcodeFiles(input)
  for (const file of lfcode) {
...
async function lfcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "lfcode"),
    ...(await Filesystem.findUp(["lfcode.json", "lfcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "lfcode"))
  }
  if (Flag.LFCODE_CONFIG) files.push(Flag.LFCODE_CONFIG)
