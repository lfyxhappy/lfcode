...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".lfcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".lfcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".lfcode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.LFCODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.LFCODE_PURE && config.plugin_origins?.length) {
