...
import lfcode from "./theme/lfcode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@lfcode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "lfcode")
        draft.active = typeof active === "string" ? active : "lfcode"
        draft.ready = false
      }),
    )
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const lfcodeOrange = RGBA.fromInts(255, 103, 0)

  return {
    theme: {
      primary: lfcodeOrange,
      secondary: lfcodeOrange,
      accent: lfcodeOrange,

      // Status colors using ANSI
      error: ansiColors.red,
      warning: ansiColors.yellow,
      success: lfcodeOrange,
      info: lfcodeOrange,
