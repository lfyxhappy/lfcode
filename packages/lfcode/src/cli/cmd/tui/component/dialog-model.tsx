...
import { createDialogProviderOptions, DialogProviderConnect } from "./dialog-provider"
...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "lfcode" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
        ...
        return [
          {
            ...
            category,
            disabled: provider.id === "lfcode" && model.id.includes("-nano"),
            footer: model.cost?.input === 0 && provider.id === "lfcode" ? "Free" : undefined,
            onSelect: () => {
      ...
      flatMap((provider) => {
        const models = pipe(
          ...
          map(([model, info]) => ({
            ...
            disabled: provider.id === "lfcode" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "lfcode" ? "Free" : undefined,
            onSelect() {
