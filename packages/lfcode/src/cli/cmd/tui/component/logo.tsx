...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const LFCODE_ORANGE = RGBA.fromInts(251, 129, 71)
  const LFCODE_GRAY = RGBA.fromInts(160, 160, 160)
          ...
          if (labelRow) {
            return (
              <box flexDirection="row" gap={1}>
                <text fg={LFCODE_GRAY} selectable={false}>{line}</text>
                <text fg={LFCODE_GRAY} selectable={false}>{ctx.shape.right[index()]}</text>
              </box>
            )
          }
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), props.ink ?? LFCODE_ORANGE, true, 0, frame(), dusk(), idleState())}
              </box>
              <box flexDirection="row">
                {renderLine(
                  ctx.shape.right[index()],
                  index(),
                  props.ink ?? LFCODE_GRAY,
                  true,
                  ctx.LEFT + GAP,
                  frame(),
                  dusk(),
