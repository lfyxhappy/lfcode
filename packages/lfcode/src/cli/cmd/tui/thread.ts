...
async function target() {
  if (typeof LFCODE_WORKER_PATH !== "undefined") return LFCODE_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start lfcode tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start lfcode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [LFCODE_PROCESS_ROLE]: "worker",
        [LFCODE_RUN_ID]: ensureRunID(),
      })
