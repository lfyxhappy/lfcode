...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    LFCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    LFCODE_WORKSPACE_ID: config.id,
    LFCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.LFCODE_EXPERIMENTAL_WORKSPACES) return
