...
export function parsePath(absPath: string): MemoryLocator | null {
  const m = normalizePath(absPath).match(/\/memory\/(global|projects|sessions)(?:\/([^/]+))?\/(.+)\.md$/)
  if (!m) return null
...
export function parseCcPath(absPath: string): MemoryLocator | null {
  const m = normalizePath(absPath).match(CC_PATH_RE)
  if (!m) return null
  const [, slug, keyRaw] = m
  return {
    scope: "cc",
...
function assertSafeKey(value: string) {
  ...
  // Reject any segment containing ".." or an absolute path — guards against
  // path traversal and absolute-path injection from caller-supplied keys.
...
function assertSafeScopeId(value: string) {
  const normalized = normalizePath(value)
  if (normalized.includes("/")) throw new Error(`buildPath: invalid path component: ${value}`)
  assertSafeKey(value)
}

export function buildPath(input: { root: string; scope: Scope; scope_id?: string; key: string }): string {
  if (input.scope_id !== undefined) assertSafeScopeId(input.scope_id)
