...
// See upstream lfcode migration note for cache-marker compatibility.
...
function limitImages(msgs: ModelMessage[]): ModelMessage[] {
  const maxImages = Flag.LFCODE_MAX_PROMPT_IMAGES
  const maxSize = Flag.LFCODE_MAX_PROMPT_IMAGE_SIZE
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "lfcode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (
    ...
    if (input.model.providerID.startsWith("lfcode")) {
      result["promptCacheKey"] = input.sessionID
