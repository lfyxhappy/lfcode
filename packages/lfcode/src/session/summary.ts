  ...
  Effect.gen(function* () {
    ...
    const diff = Effect.fn("SessionSummary.diff")(function* (input: {
      ...
      if (input.messageID || input.turns) {
        ...
        return yield* computeDiff({ messages: selectTurnWindow(messages, input) })
      }
...
function selectTurnWindow(
  ...
  input: { messageID?: MessageID; turns?: number },
) {
  const target = input.messageID
  const turns = input.turns && input.turns > 0 ? Math.floor(input.turns) : undefined
  const userIndexes = messages
    .flatMap((item, index) => (item.info.role === "user" ? [{ id: item.info.id, index }] : []))
    .sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
  if (!userIndexes.length) return []

  const endUserIndex = target
    ? userIndexes.findIndex((item) => item.id === target)
    : userIndexes.length - 1
  if (endUserIndex < 0) return []

  const startUserIndex = turns ? Math.max(0, endUserIndex - turns + 1) : endUserIndex
  const start = userIndexes[startUserIndex]?.index ?? 0
  const after = userIndexes[endUserIndex + 1]?.index
  return after === undefined ? messages.slice(start) : messages.slice(start, after)
}
