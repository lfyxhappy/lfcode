...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in lfcode, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's lfcode-native continuation messages in the same session.
          ...
          // Preserve lfcode-owned metadata on re-sync: keep any user rename
