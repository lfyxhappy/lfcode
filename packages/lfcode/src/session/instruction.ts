...
const FILES = [
  "AGENTS.md",
  ...(Flag.LFCODE_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.LFCODE_CONFIG_DIR) {
    files.push(path.join(Flag.LFCODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.LFCODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
export class Service extends Context.Service<Service, Interface>()("@lfcode/Instruction") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.LFCODE_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no LFCODE_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.LFCODE_CONFIG_DIR, Flag.LFCODE_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
