...
function buildFeedback(args: {
  ...
  if (args.kind === "missing") {
    return [
      ...
      "",
      'Write the file now with the `edit` tool using `operation:"write"`, then terminate normally.',
    ].join("\n")
  }
  ...
  return [
    ...
    TEMPLATE.replace("<TID>", args.taskId),
    "",
    'Re-write the file with the `edit` tool using `operation:"write"`, then terminate normally.',
  ].join("\n")
}
...
export async function SubagentProgressCheckerPlugin(_pluginInput: PluginInput): Promise<Hooks> {
  return {
    "actor.postStop": {
      ...
      // See packages/lfcode/src/plugin/matcher.ts.
      matcher: {
        agentType: {
          excludeOnly: [
            "checkpoint-writer",
