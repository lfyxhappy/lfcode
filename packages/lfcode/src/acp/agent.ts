...
import type { AssistantMessage, Event, LfcodeClient, SessionMessageResponse, ToolPart } from "@lfcode-ai/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: LfcodeClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `lfcode auth login` in the terminal",
      name: "Login with Lfcode",
      id: "lfcode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Lfcode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const lfcodeProvider = providers.find((p) => p.id === "lfcode")
  if (lfcodeProvider) {
    if (lfcodeProvider.models["big-pickle"]) {
      return { providerID: ProviderID.lfcode, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(lfcodeProvider.models))
    if (best) {
      return {
