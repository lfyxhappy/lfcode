...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for Lfcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal Lfcode sessions
...
lfcode acp --cwd /path/to/project
```
...
```bash
LFCODE_ENABLE_QUESTION_TOOL=1 lfcode acp
```
...
- **Terminal Integration**: Full terminal support via Lfcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | lfcode acp
```
...
ACP sessions map cleanly to Lfcode's internal session model:
