...
const Headers = Schema.Struct({
  ...
  "x-lfcode-directory": Schema.optional(Schema.String),
})
...
class Authorization extends HttpApiMiddleware.Service<Authorization>()("@lfcode/ExperimentalHttpApiAuthorization", {
  error: Unauthorized,
  security: {
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.LFCODE_SERVER_PASSWORD) return yield* effect

        const user = Flag.LFCODE_SERVER_USERNAME ?? "lfcode"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.LFCODE_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-lfcode-directory"] || process.cwd()
