...
interface MockClientState {
  ...
  advertisePromptsCapability: boolean
  advertiseResourcesCapability: boolean
...
function getOrCreateClientState(name?: string): MockClientState {
  ...
  if (!state) {
    state = {
      ...
      advertisePromptsCapability: true,
      advertiseResourcesCapability: true,
      prompts: [],
      resources: [],
      closed: false,
...
void mock.module("@modelcontextprotocol/sdk/client/index.js", () => ({
  Client: class MockClient {
    ...
    getServerCapabilities() {
      return {
        tools: {},
        ...(this._state?.advertisePromptsCapability ? { prompts: {} } : {}),
        ...(this._state?.advertiseResourcesCapability ? { resources: {} } : {}),
      }
    }
...
test("disconnect persists enabled false to project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        `${dir}/lfcode.json`,
  ...
  const text = await Bun.file(`${tmp.path}/lfcode.json`).text()
  expect(JSON.parse(text).mcp["disc-server"].enabled).toBe(false)
})

test("connect persists enabled true for Claude project MCP via local override", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        `${dir}/.claude.json`,
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await Effect.runPromise(
        MCP.Service.use((mcp) =>
          Effect.gen(function* () {
            lastCreatedClientName = "filesystem"
            getOrCreateClientState("filesystem")
            yield* mcp.connect("filesystem")
          }),
        ).pipe(Effect.provide(MCP.defaultLayer)),
      )
  ...
  const text = await fs.readFile(path.join(tmp.path, ".lfcode", "lfcode.jsonc"), "utf8")
  expect(JSON.parse(text).mcp.filesystem.enabled).toBe(true)
})

test("connect persists enabled true for home Claude MCP to global config", async () => {
  const source = path.join(Global.Path.home, ".claude.json")
  const globalConfigPath = path.join(Global.Path.config, "lfcode.jsonc")
  const originalClaude = await fs.readFile(source, "utf8").catch(() => undefined)
  const originalGlobal = await fs.readFile(globalConfigPath, "utf8").catch(() => undefined)
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        await Effect.runPromise(
          MCP.Service.use((mcp) =>
            Effect.gen(function* () {
              lastCreatedClientName = "context7"
              getOrCreateClientState("context7")
              yield* mcp.connect("context7")
            }),
          ).pipe(Effect.provide(MCP.defaultLayer)),
        )
        await Instance.dispose()
    ...
    expect(JSON.parse(text).mcp.context7.enabled).toBe(true)
  } finally {
    if (originalClaude === undefined) {
      await fs.rm(source, { force: true }).catch(() => undefined)
    } else {
      await Bun.write(source, originalClaude)
    }
    if (originalGlobal === undefined) {
      await fs.rm(globalConfigPath, { force: true }).catch(() => undefined)
    } else {
      await Bun.write(globalConfigPath, originalGlobal)
    }
  }
})
...
test(
  "prompts() memoizes unsupported prompt listings after method-not-found",
  withInstance(
    {
      "prompt-missing-server": {
        type: "local",
        command: ["echo", "test"],
      },
    },
    (mcp) =>
      Effect.gen(function* () {
        lastCreatedClientName = "prompt-missing-server"
        const serverState = getOrCreateClientState("prompt-missing-server")
        serverState.listPromptsShouldFail = true

        yield* mcp.add("prompt-missing-server", {
          type: "local",
          command: ["echo", "test"],
        })
...
test(
  "resources() skips clients that do not advertise resources capability",
  withInstance(
    {
      "resource-capless-server": {
        type: "local",
        command: ["echo", "test"],
      },
    },
    (mcp) =>
      Effect.gen(function* () {
        lastCreatedClientName = "resource-capless-server"
        const serverState = getOrCreateClientState("resource-capless-server")
        serverState.advertiseResourcesCapability = false

        yield* mcp.add("resource-capless-server", {
          type: "local",
          command: ["echo", "test"],
        })
