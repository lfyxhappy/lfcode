...
// the live router + a real key in ~/.config/lfcode/lfcode.json). Run with:
...
function realConfig() {
  const home = path.join(os.homedir(), ".config", "lfcode", "lfcode.json")
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .lfcode/workflows.
            const wfDir = path.join(dir, ".lfcode", "workflows")
