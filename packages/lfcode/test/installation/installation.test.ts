...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @lfcode/cli-ai is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@lfcode/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @lfcode/cli-ai is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@lfcode/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @lfcode/cli-ai is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@lfcode/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @lfcode/cli-ai", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
    ...
    test("reads version from xiaomi npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          ...
          expect(req.url).toContain(encodeURIComponent("@lfcode/cli-ai"))
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package and registry", async () => {
      ...
      expect(capturedArgs).toContain("@lfcode/cli-ai@2.0.0")
