...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(
    path.join(process.env["LFCODE_TEST_TMPDIR_ROOT"] ?? os.tmpdir(), "lfcode-test-" + Math.random().toString(36).slice(2)),
  )
  ...
  if (options?.git) {
    ...
    await $`git config user.email "test@lfcode.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "lfcode.json"),
      JSON.stringify({
...
export function tmpdirScoped(options?: { git?: boolean; config?: Partial<Config.Info> }) {
  return Effect.gen(function* () {
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@lfcode.test")
    ...
    if (options?.config) {
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://lfcode.ai/config.json", ...options.config }),
        ),
      )
    }
