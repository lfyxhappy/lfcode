...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under lfcode.", () => {
    expect(paramToAttributeKey("name")).toBe("lfcode.name")
    expect(paramToAttributeKey("slug")).toBe("lfcode.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["lfcode.sessionID"]).toBeUndefined()
    expect(attrs["lfcode.messageID"]).toBeUndefined()
    expect(attrs["lfcode.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under lfcode. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["lfcode.name"]).toBe("exa")
