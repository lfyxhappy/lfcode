    ...
    await withoutWatcher(() =>
      Instance.provide({
        ...
        fn: async () => {
          ...
          const res = await app.request(`/usage?range=all&source=lfcode&search=usage-session`)
          ...
          const filtered = await app.request(`/usage?range=all&provider=anthropic&source=lfcode&search=usage-session`)
      ...
      await withoutWatcher(() =>
        Instance.provide({
          ...
          fn: async () => {
            ...
            const res = await app.request(`/usage?range=all&source=lfcode&search=paged-usage-session&limit=1`)
            expect(res.status).toBe(200)
            const body = (await res.json()) as {
