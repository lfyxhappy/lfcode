...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "lfcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "lfcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "lfcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "lfcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["lfcode.json", "lfcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "lfcode.json"),
        path.join(project, "lfcode.jsonc"),
      ])
    })
  })
