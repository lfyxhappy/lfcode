...
describe("parsePath", () => {
  ...
  test("windows path separators are normalized during parse", () => {
...
describe("buildPath", () => {
  ...
  test("global free", () => {
    expect(buildPath({ root: "/data/memory", scope: "global", key: "tooling" })).toBe(path.join("/data/memory", "global", "tooling.md"))
  })
  ...
  test("rejects windows-style traversal in key", () => {
    expect(() =>
      buildPath({ root: "C:\\data\\memory", scope: "sessions", scope_id: "ses_abc", key: "..\\escape" }),
    ).toThrow(/invalid path component/)
  })

  test("rejects path separators in scope_id", () => {
    expect(() =>
      buildPath({ root: "C:\\data\\memory", scope: "sessions", scope_id: "ses\\abc", key: "checkpoint" }),
    ).toThrow(/invalid path component/)
  })
})
