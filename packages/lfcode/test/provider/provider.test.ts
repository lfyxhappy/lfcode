...
test("models.dev normalization fills required response fields", () => {
  ...
  expect(model.capabilities.native_web).toBe(false)
  expect(model.capabilities.toolcall).toBe(true)
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "lfcode" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".lfcode", "plugin")
...
test("opencode and lfcode-go providers are disabled by MimoFreeAuthPlugin", async () => {
  ...
  // MimoFreeAuthPlugin always pushes opencode/lfcode-go into disabled_providers,
  ...
  expect(providers[ProviderID.make("lfcode-go")]).toBeUndefined()
