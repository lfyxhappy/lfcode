...
Both `action` and `resource` use lfcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.lfcode` files is intentionally deferred until `.lfcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether lfcode permits an operation through its managed execution paths.
