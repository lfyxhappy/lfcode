...
- Canonical tagged tool file sources in `@lfcode-ai/llm`.
...
- Spill oversized model-facing tool text into globally unique files under Lfcode's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `LFCODE_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
