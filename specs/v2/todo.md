...
The lfcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@lfcode-ai/llm` tool loop after replacing its
...
- plugins get global "lfcode" instance like in that post i showed
- lfcode instance has stuff like `lfcode.session.prompt()` or
  `lfcode.tool.register({...})`
...
- decide whether to preserve deprecated `@lfcode-ai/llm` orchestration exports
