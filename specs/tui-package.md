...
Move the canonical Lfcode terminal application from
`packages/lfcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/lfcode`,
`packages/cli`, or `@lfcode-ai/core`.

The SDK is the TUI's Lfcode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/lfcode`.
- Do not replace private `packages/lfcode` imports with `@lfcode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@lfcode-ai/tui`.
...
- Lfcode domain data displayed by the TUI
...
```text
packages/lfcode/src/cli/cmd/tui
```

Its private dependency on `packages/lfcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/lfcode/src/*`.
...
Status: Completed. The private `@lfcode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@lfcode-ai/tui`.
...
- The package has no dependency on `packages/lfcode`, `@lfcode-ai/cli`, or
  `@lfcode-ai/core`.
...
primitives now live in `@lfcode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@lfcode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Lfcode globals or process state;
...
Replace process-global Lfcode state with resolved TUI inputs.
...
`@lfcode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/lfcode` initially.
...
canonical in `@lfcode-ai/tui`. Configured references resolve through the new
...
adapters rather than Lfcode domain implementations.

Make the SDK the only Lfcode domain boundary used by the TUI.
...
- Domain-facing TUI code imports Lfcode data and operations only from
  `@lfcode-ai/sdk`.
...
`@lfcode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/lfcode`.
...
- The package has no imports from `packages/lfcode`, `packages/cli`, or
  `@lfcode-ai/core`.
...
public `@lfcode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/lfcode`.
- Keep the legacy embedded worker and server startup in `packages/lfcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@lfcode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/lfcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@lfcode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@lfcode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Lfcode domain state.
...
cd packages/lfcode && bun typecheck
...
rg '@lfcode-ai/core|packages/lfcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/lfcode/src packages/lfcode/test
```
...
- Build the current-platform `packages/lfcode` binary.
