...
- shell 提示词要求把命令、原始输出、凭据与私有路径留在结构化 tool part；结果文本仅保留结论和关键诊断。
- shell、截断输出、后台任务日志与流式 assistant 文本均在持久化前做 credential-shaped 内容脱敏。
- 已完成使用版打包和启动校验；仍待用真实模型回合确认不会重复叙述工具过程。
